...
import { Search, ChevronUp, ChevronDown, Download, Calendar as CalendarIcon, TrendingUp, Users, Repeat, DollarSign } from 'lucide-react';
...
export default function Reports() {
    ...
    const [sortField, setSortField] = useState<'name' | 'totalSubscriptions' | 'latestSubscriptionDate' | 'totalSpent' | null>(null);
    const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc');
    const [dateRange, setDateRange] = useState<'all' | 'this_month' | 'last_month' | '3_months' | '6_months' | 'this_year'>('all');
    ...
    const handleSort = (field: 'name' | 'totalSubscriptions' | 'latestSubscriptionDate' | 'totalSpent') => {
        if (sortField === field) {
    ...
    const handleExportCSV = () => {
        const headers = ['Name', 'Email', 'Total Subscriptions', 'Total Spent', 'First Subscription', 'Latest Subscription', 'Latest Package', 'Status'];
        const csvContent = [
            headers.join(','),
        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                    <Card className="bg-black/40 backdrop-blur-xl border-white/10">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-base font-medium text-muted-foreground">Repeat Customers</CardTitle>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                    <Card className="bg-black/40 backdrop-blur-xl border-white/10">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-base font-medium text-muted-foreground">Retention Rate</CardTitle>
                                    ...
                                    <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">{customer.totalSubscriptions} orders</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-lg text-green-400">{formatCurrency(customer.totalSpent)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                    ...
                                    <TableHeader>
                                        <TableRow className="hover:bg-white/5 border-white/10">
                                            ...
                                            <TableHead onClick={() => handleSort('name')} className="cursor-pointer">
                                                Name {sortField === 'name' && (sortOrder === 'asc' ? <ChevronUp className="inline h-3 w-3" /> : <ChevronDown className="inline h-3 w-3" />)}
                                            </TableHead>
                                            <TableHead onClick={() => handleSort('totalSubscriptions')} className="cursor-pointer text-center">
                                                Orders {sortField === 'totalSubscriptions' && (sortOrder === 'asc' ? <ChevronUp className="inline h-3 w-3" /> : <ChevronDown className="inline h-3 w-3" />)}
                                            </TableHead>
                                            <TableHead onClick={() => handleSort('totalSpent')} className="cursor-pointer text-right">
                                                Spent {sortField === 'totalSpent' && (sortOrder === 'asc' ? <ChevronUp className="inline h-3 w-3" /> : <ChevronDown className="inline h-3 w-3" />)}
                                            </TableHead>
